...
| column | meaning |
|---|---|
| on_hand_units | current stock (shared pool, not broken out by physical store) |
...
- **days_of_cover** — on-hand stock ÷ the *current* daily sell-through rate. This deliberately uses the higher of the trailing 4-week and 8-week rates rather than the spike-discounted figure: discounting a spike when sizing an order is defensible, but stock depletes at the rate it is actually selling, so using the smoothed number would understate risk exactly when it is highest. When the projected gap between cover and lead time reaches **7 days or more** — meaning roughly a week with nothing on the shelf — the SKU is flagged **urgent** regardless of forecast confidence, since it's a timing risk, not a demand-accuracy one. A bare "cover < lead time" test was tried first and fired on more than half the catalog, including SKUs running dry a single day early, which made the flag noise rather than signal.
