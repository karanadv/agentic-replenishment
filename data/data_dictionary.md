...
**Regenerate this data:** `python3 data/generate_data.py` — reproduces all three CSVs byte-for-byte (fixed random seed).
...
Note: full **fulfillment orchestration** is out of scope for this prototype. That explicitly includes **BOPIS** (buy online, pick up in store), **BORIS** (buy online, return in store), ship-from-store and cross-store/warehouse transfers — none of which are modelled in this data. BOPIS would reserve units against a store before collection, and BORIS would return units to stock at an unpredictable time and place; neither is representable while `on_hand_units` is a single pooled figure. Orchestration — it requires per-store inventory and an order-routing layer, which is a different system from replenishment planning. The `days_of_cover` / urgent flag is a deliberately lightweight signal that surfaces *when* those workarounds would be needed, without simulating the routing itself. See the decisions README for the full reasoning.
